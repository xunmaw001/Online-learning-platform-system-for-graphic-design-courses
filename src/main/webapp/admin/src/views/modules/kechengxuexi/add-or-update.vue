<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'&& !ro.kechengwenjian" label="课程文件" prop="kechengwenjian">
          <file-upload
          tip="点击上传课程文件"
          action="file/upload"
          :limit="1"
...
export default {
  ...
  methods: {
	...
	this.ruleForm.kechengneirong = this.ruleForm.kechengneirong.replace(reg,'../../../ssm8s7c9/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `kechengxuexi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.kechengxuexiCrossAddOrUpdateFlag = false;
						 this.parent.search();
