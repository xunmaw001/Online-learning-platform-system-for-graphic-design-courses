<template>
  <el-breadcrumb class="app-breadcrumb" separator="/" style="height:65px;backgroundColor:rgba(38, 69, 101, 0.71);borderRadius:20px;padding:0px 20px 0px 20px;boxShadow:0px 0px 0px #f903d4;borderWidth:10px 15px;borderStyle:solid double;borderColor:rgba(255, 255, 255, 1);">
    <transition-group name="breadcrumb" class="box" :style="1==1?'justifyContent:flex-start;':1==2?'justifyContent:center;':'justifyContent:flex-end;'">
      <el-breadcrumb-item v-for="(item,index) in levelList" :key="item.path">
        <span v-if="item.redirect==='noRedirect'||index==levelList.length-1" class="no-redirect" style="color:rgba(215, 215, 215, 1)">{{ item.name }}</span>
    ...
    breadcrumbStyleChange(val) {
      this.$nextTick(()=>{
        ...
        document.querySelectorAll('.app-breadcrumb .el-breadcrumb__inner .no-redirect').forEach(el=>{
          el.style.color = "rgba(215, 215, 215, 1)"
        })
